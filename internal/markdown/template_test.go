...
func TestObjectSeparators_GetSeparatorForLevel(t *testing.T) {
	tests := []struct {
		name       string
		...
		{
			name: "specific level overrides all-levels rule",
			separators: []ObjectSeparator{
				{Level: -1, Style: SeparatorStyleBlank, Count: 2},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sep := cfg.GetSeparatorForLevel(tt.level)
			if tt.wantNil {
				if sep != nil {
					t.Errorf("Expected nil separator, got: %+v", sep)
				}
			} else {
				if sep == nil {
					t.Error("Expected separator, got nil")
				} else if sep.Style != tt.wantStyle {
					t.Errorf("Expected style %q, got %q", tt.wantStyle, sep.Style)
				}
			}
		})
	}
...
func TestValidateSeparator(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "valid separator with level 0",
			separator: ObjectSeparator{Level: 0, Style: SeparatorStyleBlank, Count: 1},
			wantError: false,
		},
		{
			name:      "valid separator with level -1 (all levels)",
			separator: ObjectSeparator{Level: -1, Style: SeparatorStyleLine},
			wantError: false,
		},
		{
			name:      "invalid level < -1",
			separator: ObjectSeparator{Level: -2, Style: SeparatorStyleBlank},
			wantError: true,
			errorMsg:  "level must be >= -1",
		},
		{
			name:      "invalid style",
			separator: ObjectSeparator{Level: 1, Style: "invalid"},
			wantError: true,
			errorMsg:  "invalid style",
		},
		{
			name:      "empty style defaults to none",
			separator: ObjectSeparator{Level: 1, Style: ""},
			wantError: false,
		},
		{
			name:      "blank style with zero count (defaults to 1)",
			separator: ObjectSeparator{Level: 1, Style: SeparatorStyleBlank, Count: 0},
			wantError: false,
		},
	}
